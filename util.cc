...
// Author: Yangfeng Ji
// Date: 09-02-2016
// Time-stamp: <yangfeng 10/22/2017 15:43:54>
...
int load_model(string fname, ParameterCollection& model){
  TextFileLoader loader(fname);
  loader.populate(model);
  return 0;
}

int save_model(string fname, ParameterCollection& model){
...
bool check_doc(const Doc& doc){
  bool pass = false;
  for (int i = 0; i < doc.sents.size(); i++){
    for (int j = 0; j < doc.sents[i].size(); j++){
      if (doc.sents[i][j].tidx > 0)
	pass = true;
    }
  }
  return pass;
